...
class FilariaInterController extends Controller
{
    public function ViewPage()
    {
        ...
        return view('pages.FilariaInter')->with(['readOnly' => $readOnly, 'readWrite' => $readWrite]);
    }
    ...
    public function ViewPage2()
    {
        ...
        return view('pages.FilariaViewDet')->with(['readOnly' => $readOnly, 'readWrite' => $readWrite]);
    }
    ...
    public function passportLoadDrp(Request $request)
    {
        $res = DB::select("select max(ps_id) as id, ps_passp_no from tbl_phlebotomy_sample
                where ps_filaria_barcode is not null
    ...
    public function filariaViewDetLoadData(Request $request)
    {
        $id = $request->id;
