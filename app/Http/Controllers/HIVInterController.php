...
class HIVInterController extends Controller
{
    public function ViewPage()
    {
        ...
        return view('pages.HIVInter')->with(['readOnly' => $readOnly, 'readWrite' => $readWrite]);
    }
    ...
    public function ViewPage2()
    {
        ...
        return view('pages.HivViewDet')->with(['readOnly' => $readOnly, 'readWrite' => $readWrite]);
    }
    ...
    public function passportLoadDrp(Request $request)
    {
        $res = DB::select("select max(ps_id) as id, ps_passp_no from tbl_phlebotomy_sample
                where ps_hiv_barcode is not null
    ...
    public function HivViewDetLoadData(Request $request)
    {
        $id = $request->id;
