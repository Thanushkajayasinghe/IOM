$(document).ready(function () {
    ...
    function getPendingTokens() {
        ...
        $.ajax({
            url: '/IOM/PendingTokensURL',
            type: 'post',
            dataType: 'json',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            data: { country: location.href.substring(location.href.lastIndexOf('/') + 1) },
    ...
    $('#next').on('click', function () {
        ...
        if (arr.length == 0) {
            ...
            $.ajax({
                ...
                data: {
                    preToken: preToken, country: location.href.substring(location.href.lastIndexOf('/') + 1)
                },
                success: function (data) {
                    ...
                    if (data == "No Token") {
                        ...
                        $.ajax({
                            ...
                            data: {
                                token: data, country: location.href.substring(location.href.lastIndexOf('/') + 1)
                            },
                            success: function (data) {
                                ...
                                $.each(data, function (key, val) {
                                    ...
                                    $('#appbody').append('<tr class="appNum"> <td>' + val.appointment_no + '</td> <td style="display:none">' + val.member_type + '</td> </tr>');
                                });
    ...
    $(document).on('click', "tr.appNum", function () {
        ...
        if (!$(this).hasClass('rowSaved')) {
            ...
            $.ajax({
                url: '/IOM/CheckCallAgainAppNoURL',
                type: 'post',
                dataType: 'json',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                success: function (data) {
                    if (data.result == false) {
                        $.ajax({
                            url: '/IOM/LoadAppointmentNoDataURL',
                            type: 'post',
                            dataType: 'json',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            ...
                            success: function (data) {
                                ...
                                $('#AddressLocal').val((data.address1 || "") + ' ' + (data.address2 || "") + ' ' + (data.city || ""));
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.result, function (key, val) {
                    $('#appNotAvblBody').append('<tr class="appNotAvblNum"> <td attr-no="' + val.registration_status + '">' + val.token_no + '</td> </tr>');
                });
            }
        });
               ...
               $('#appbody').html('');
                 if (data.count == undefined) {
                    ...
                    $.each(combinedArray, function (key, val) {
                        arr[key] = val;
                        var isDuplicate = uniqueAppointments.some(function (item) {
                       ...
                       if (!isDuplicate) {
                            ...
                            Array_member_type.push(val.member_type);
                            var rowSaved = '';
                            if (data.count.some(countItem => countItem.appointment_no === val.appointment_no)) {
                                $('#appbody').append('<tr class="appNum  rowSaved"> <td>' + val.appointment_no + '</td> <td style="display:none">' + val.member_type + '</td> </tr>');
                            }
                            else{
                                $('#appbody').append('<tr class="appNum"> <td>' + val.appointment_no + '</td> <td style="display:none">' + val.member_type + '</td> </tr>');
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                $('#appbody tr.appNum').each(function () {
                                    var $this = $(this);
                                    if (appnox.trim() == $this.find('td:nth-child(1)').text().trim()) {
    ...
    $('#btnUpdateDetails').on('click', function () {
        ...
        var win = window.open('/IOM/UpdateAppointment/' + country, '_blank');
